    ...
    if (nowPage == "10-expense.html") {
        collection.load(function () {
            ...
            for (var i = 0; i < expenses.length; i++) {
                $("tbody").append(`<tr><td>${expenses[i].date}</td><td>${expenses[i].name}</td><td>${expenses[i].price}</td><td><button type="button" class="btn btn-danger" onclick="removeRecord('${expenses[i]._id}')"><i class="fas fa-trash-alt"></i></button></td></tr>`);
            }
        });
    }
    ...
    if (nowPage == "search.html") {
        $("#submit").on("click", function () {
            ...
            if (expenses.length === 0) {
                ...
                for (var i = 0; i < expenses.length; i++) {
                    $("#expenses tbody").append(`<tr><td>${expenses[i].date}</td><td>${expenses[i].name}</td><td>${expenses[i].price}</td><td><button type="button" class="btn btn-danger" onclick="removeRecord('${expenses[i]._id}')"><i class="fas fa-trash-alt"></i></button></td></tr>`);
